...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    elif num_classes == 6 or dataset == 'ks':
      self.names = ks_class_name
    elif dataset == 'gta':
  ...
  def add_coco_bbox(self, bbox, cat, conf=1, show_txt=True, img_id='default'): 
    ...
    colors = [(255,0,0),(0,255,0),(0,0,255),(127,127,0),(0,127,127),(127,0,127)]
    cv2.rectangle(
      self.imgs[img_id], (bbox[0], bbox[1]), (bbox[2], bbox[3]), colors[cat], 2)
    # cv2.rectangle(
  ...
  def save_all_imgs(self, path='../exp/ctdet/ks_result/', prefix='', genID=False):
    if genID:
      try:
    ...
    for i, v in self.imgs.items(): # i为key，v为value；区别于enumerate()
      cv2.imwrite(path + '/{}{}'.format(prefix, i), v)
...
ks_class_name = ['shu', 'tuo', 'fang', 'ks', 'other', 'frac']
