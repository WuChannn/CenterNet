...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('task', default='ctdet',
                             help='ctdet | ddd | multi_pose | exdet') # exdet: ExtremeNet
    self.parser.add_argument('--dataset', default='ks',
                             help='coco | kitti | coco_hp | pascal | ks')
    self.parser.add_argument('--exp_id', default='default') # exp/ctdet文件夹中存储该experiment的文件夹名称
    self.parser.add_argument('--test', action='store_true') # 触发action时test为真，不触发时test则为假
    ...
    self.parser.add_argument('--num_epochs', type=int, default=1400,
  ...
  def init(self, args=''):
    default_dataset_info = {
      'ctdet': {'default_resolution': [512, 512], 'num_classes': 6, 
                'mean': [0.589917, 0.611671, 0.625294], 'std': [0.196148, 0.188748, 0.186552],
                'dataset': 'ks'},
